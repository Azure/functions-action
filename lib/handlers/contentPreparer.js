...
class ContentPreparer {
    ...
    derivePublishMethod(state, packageType, osType, sku, authenticationType, params) {
        ...
        if (authenticationType == authentication_type_1.AuthenticationType.Scm) {
            ...
            if (!!params.sku && function_sku_1.FunctionSkuUtil.FromString(params.sku) === function_sku_1.FunctionSkuConstant.FlexConsumption) {
                utils_1.Logger.Info('Will use Kudu https://<scmsite>/api/publish to deploy since Flex consumption plan is detected.');
                return publish_method_1.PublishMethodConstant.OneDeployFlex;
            }
        ...
        // Flex Consumption plan uses api/publish endpoint
        if (osType === runtime_stack_1.RuntimeStackConstant.Linux && sku === function_sku_1.FunctionSkuConstant.FlexConsumption) {
            utils_1.Logger.Info('Will use Kudu https://<scmsite>/api/publish to deploy since Flex consumption plan is detected.');
            return publish_method_1.PublishMethodConstant.OneDeployFlex;
        }
