...
'use strict';
import { Logger, DEFAULT_CRYPTO_IMPLEMENTATION, Authority, UrlString, CacheOutcome, Constants, ProtocolMode } from '@azure/msal-common';
...
import { invalidResource } from '../error/ManagedIdentityErrorCodes.mjs';
...
class ManagedIdentityApplication {
    ...
    async acquireToken(managedIdentityRequestParams) {
        const resourceUrlString = new UrlString(managedIdentityRequestParams.resource.replace("/.default", ""));
        try {
            resourceUrlString.validateAsUri();
        }
        catch (e) {
            throw createManagedIdentityError(invalidResource);
        }
