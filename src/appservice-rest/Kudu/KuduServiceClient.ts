...
export class KuduServiceClient {
    ...
    public async beginRequest(
        ...
        if (!!this._cookie) {
            core.debug(
                `setting affinity cookie ${JSON.stringify(this._cookie)}`
            );
            request.headers["Cookie"] = this._cookie;
        }

        let retryCount =
            reqOptions && util.isNumber(reqOptions.retryCount)
                ? reqOptions.retryCount
                : 5;

        while (retryCount >= 0) {
            try {
                let httpResponse = await this._webClient.sendRequest(
                    request,
                    reqOptions
                );
                if (httpResponse.headers["set-cookie"] && !this._cookie) {
                    ...
                    core.debug(
                        `loaded affinity cookie ${JSON.stringify(this._cookie)}`
                    );
                }
                ...
                if (
                    ...
                    core.warning(
                        "To use a certificate in App Service, the certificate must be signed by a trusted certificate authority. If your web app gives you certificate validation errors, you're probably using a self-signed certificate and to resolve them you need to export variable named ACTIONS_AZURE_REST_IGNORE_SSL_ERRORS to the value true."
                    );
                }

                if (
                    retryCount > 0 &&
                    exceptionString.indexOf("Request timeout") != -1 &&
                    (!reqOptions || reqOptions.retryRequestTimedout)
                ) {
                    core.debug(
