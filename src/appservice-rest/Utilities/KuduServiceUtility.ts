...
export class KuduServiceUtility {
    ...
    public async updateDeploymentStatus(
        ...
        try {
            let requestBody = this._getUpdateHistoryRequest(
                taskResult,
                DeploymentID,
                customMessage
            );
    ...
    public async deployUsingZipDeploy(
        packagePath: string,
        customMessage?: any
    ): Promise<string> {
        try {
            ...
            var deploymentMessage = this._getUpdateHistoryRequest(
                null,
                null,
                customMessage
            ).message;
            queryParameters.push(
                "message=" + encodeURIComponent(deploymentMessage)
            );
            let deploymentDetails = await this._webAppKuduService.zipDeploy(
                packagePath,
                queryParameters
            );
    ...
    public async deployUsingWarDeploy(
        packagePath: string,
        customMessage?: any,
        targetFolderName?: any
    ): Promise<string> {
        try {
            console.log("Package deployment using WAR Deploy initiated.");

            let queryParameters: Array<string> = ["isAsync=true"];

            if (targetFolderName) {
                queryParameters.push(
                    "name=" + encodeURIComponent(targetFolderName)
                );
            }
            ...
            let deploymentDetails = await this._webAppKuduService.warDeploy(
                packagePath,
                queryParameters
            );
            await this._processDeploymentResponse(deploymentDetails);
    ...
    public async postZipDeployOperation(
        oldDeploymentID: string,
        activeDeploymentID: string
    ): Promise<void> {
        try {
            core.debug(
                `ZIP DEPLOY - Performing post zip-deploy operation: ${oldDeploymentID} => ${activeDeploymentID}`
            );
            let manifestFileContent =
                await this._webAppKuduService.getFileContent(
                    `${deploymentFolder}/${oldDeploymentID}`,
                    manifestFileName
                );
            if (!!manifestFileContent) {
                let tempManifestFile: string = path.join(
                    `${process.env.RUNNER_TEMP}`,
                    manifestFileName
                );
                fs.writeFileSync(tempManifestFile, manifestFileContent);
                await this._webAppKuduService.uploadFile(
                    `${deploymentFolder}/${activeDeploymentID}`,
                    manifestFileName,
                    tempManifestFile
                );
            }
            ...
            core.debug(
                `Failed to execute post zip-deploy operation: ${JSON.stringify(
                    error
                )}.`
            );
        }
    ...
    private async _processDeploymentResponse(
        ...
        try {
            var kuduDeploymentDetails =
                await this._webAppKuduService.getDeploymentDetails(
                    deploymentDetails.id
                );
            core.debug(
                `logs from kudu deploy: ${kuduDeploymentDetails.log_url}`
            );
            ...
            if (deploymentDetails.status == KUDU_DEPLOYMENT_CONSTANTS.FAILED) {
                ...
                console.log(
                    "Deploy logs can be viewed at %s",
                    kuduDeploymentDetails.log_url
                );
            }
        } catch (error) {
            core.debug(
                `Unable to fetch logs for kudu Deploy: ${JSON.stringify(error)}`
            );
        }
    ...
    private async _printZipDeployLogs(log_url: string): Promise<void> {
        ...
        var deploymentLogs = await this._webAppKuduService.getDeploymentLogs(
            log_url
        );
    ...
    private _getUpdateHistoryRequest(
        ...
        if (
            deploymentLogType.toLowerCase() === "deployment" &&
            isDeploymentSuccess
        ) {
            active = true;
        }
        ...
        return {
            id: deploymentID,
            active: active,
            status: isDeploymentSuccess
                ? KUDU_DEPLOYMENT_CONSTANTS.SUCCESS
                : KUDU_DEPLOYMENT_CONSTANTS.FAILED,
    ...
    public async deployWebAppImage(
        appName: string,
        images: string,
        isLinux: boolean
    ) {
        try {
            core.debug(
                `DeployWebAppImage - appName: ${appName}; images: ${images}; isLinux:${isLinux}`
            );
            ...
            let headers = isLinux
                ? { LinuxFxVersion: `DOCKER|${images}` }
                : { WindowsFxVersion: `DOCKER|${images}` };
